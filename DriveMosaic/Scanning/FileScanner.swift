...
final class FileScanner: Sendable {
    ...
    /// Maximum number of individual file children to keep per directory.
    /// Beyond this, remaining files are consolidated into a single "Other files" node.
    /// This prevents millions of tiny files from exploding memory and render time.
    private static let maxFileChildrenPerDir = 50
    ...
    func scan(
        ...
        var scannedCount = 0
        let progressInterval = 200 // Report progress every N files
        ...
        // Track cumulative size of pruned small files per directory
        var prunedSizeMap: [String: UInt64] = [:]
        var prunedCountMap: [String: Int] = [:]
        var sizeMap: [String: UInt64] = [:]
        var restrictedPaths: Set<String> = []

        while let entry = fts_read(fts) {
            // Check for cancellation every iteration so cancel/rescan responds immediately
            if Task.isCancelled {
            ...
            case FTS_F:
                ...
                let mtime = entry.pointee.fts_statp.pointee.st_mtimespec
                let modDate = Date(timeIntervalSince1970: Double(mtime.tv_sec) + Double(mtime.tv_nsec) / 1_000_000_000)
                let parentPath = (entryPath as NSString).deletingLastPathComponent

                // Only keep individual file nodes up to the per-directory limit.
                // Beyond that, accumulate into a consolidated "Other files" bucket.
                let currentFileCount = childrenMap[parentPath]?.filter({ !$0.isDirectory }).count ?? 0
                if currentFileCount < Self.maxFileChildrenPerDir {
                    ...
                    // Accumulate into pruned bucket
                    prunedSizeMap[parentPath, default: 0] += fileSize
                    prunedCountMap[parentPath, default: 0] += 1
                }
            ...
            case FTS_DP:
                ...
                var children = childrenMap[entryPath] ?? []

                // Add consolidated node for pruned files if any
                let prunedSize = prunedSizeMap[entryPath] ?? 0
                let prunedCount = prunedCountMap[entryPath] ?? 0
                if prunedSize > 0 {
                    let consolidated = TransferNode(
                        name: "\(prunedCount.formatted()) other files",
                        path: entryPath + "/__consolidated__",
                        isDirectory: false,
                        size: prunedSize,
                        children: [],
                        isRestricted: false,
                    ...
                    children.append(consolidated)
                    prunedSizeMap.removeValue(forKey: entryPath)
                    prunedCountMap.removeValue(forKey: entryPath)
                }

                let dirSize = children.reduce(UInt64(0)) { $0 + $1.size }
                sizeMap[entryPath] = dirSize

                // Directory mtime from stat
                let dirMtime = entry.pointee.fts_statp.pointee.st_mtimespec
                let dirModDate = Date(timeIntervalSince1970: Double(dirMtime.tv_sec) + Double(dirMtime.tv_nsec) / 1_000_000_000)
